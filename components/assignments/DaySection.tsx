...
export default function DaySection({
            ...
            const enhanced = await Promise.all(
                assignments.map(async (assignment) => {
                    if (assignment.periodId) {
                        try {
                            const periodInfo = await getPeriodById(
                                assignment.periodId,
                            );
                            return {
                                ...assignment,
    ...
    useEffect(() => {
        if (
            memoizedHighlight &&
            assignments.some((a) => a.id === memoizedHighlight)
        ) {
                ...
                <Animated.View layout={Layout.springify().mass(0.5)}>
                    ...
                    >
                        {Object.entries(groupedByCourse).map(
                            ([courseKey, courseAssignments], index) => (
                                <Animated.View
                                    key={courseKey}
                                    ...
                                    style={[
                                        ...
                                        index ===
                                            Object.entries(groupedByCourse)
                                                .length -
                                                1 && styles.lastCourseSection,
                                    ]}
                                >
                                    <CourseSection
                                        courseCode={
                                            courseAssignments[0].courseCode ||
                                            courseKey
                                        }
                                        courseName={
                                            courseAssignments[0].courseName ||
                                            t('assignments').common
                                                .uncategorized
                                        }
