...
export default function DayView() {
  ...
  const [academicCalendar, setAcademicCalendar] = useState<GraficPeGroup | null>(null);
  ...
  const selectedAcademicPeriod = useMemo(
    () => findAcademicPeriodForDate(academicCalendar?.periods || [], selectedDate),
    [academicCalendar, selectedDate],
  );
  const regularClassesSuppressed = academicPeriodSuppressesClasses(selectedAcademicPeriod);
  const nextClassSearchDate = useMemo(
    () =>
      regularClassesSuppressed && selectedAcademicPeriod
        ...
        : selectedDate,
    [regularClassesSuppressed, selectedAcademicPeriod, selectedDate],
  );
  const nextClassCacheKey = useMemo(
    () =>
      buildNextClassCacheKey({
        ...
        sharedAcademicPeriod: regularClassesSuppressed ? selectedAcademicPeriod : null,
      }),
    [
      nextClassSearchDate,
  ...
  const scheduleOverride = useMemo(() => {
    ...
    const itemWithReason = todaySchedule.find(
      (item) => typeof item?.recoveryReason === 'string' && item.recoveryReason.trim().length > 0,
    );
  ...
  const shouldShowEmptyState = (isWeekend && !recoveryDay) || visibleTodaySchedule.length === 0;
  ...
  const assessmentExamEvents = shouldShowEmptyState ? [] : selectedDateExamEvents;
  ...
  const scrollToDate = useCallback(
    (date: Date) => {
      ...
      const dateIndex = weekDates.findIndex((d) => d.date.toDateString() === date.toDateString());
    ...
    (items: ScheduleItem[], targetDate: Date): ScheduleItem[] => {
      ...
      const reasonItem = safeItems.find(
        (item) =>
          item &&
          (item.period === 'recovery-info' ||
            (item.recoveryReason && item.recoveryReason.trim().length > 0)),
      );
        ...
        <Animated.View style={[styles.timeIndicator, animatedStyle]}>
          ...
          <Animated.Text style={[styles.timeLeftText, animatedTimeStyle]}>
            {(() => {
              ...
              const diffInMinutes = Math.ceil((endTime.getTime() - now.getTime()) / (1000 * 60));
              ...
              {weekDatesWithAssignments.map((date, index) => (
                ...
                >
                  {date.totalAssignments > 0 && (
                    <View
                      style={[
                        index === weekDatesWithAssignments.length - 1
                  ...
                  {date.isToday && date.date.getDate() !== selectedDate.getDate() && (
            ...
            <View style={styles.assessmentSection}>
              ...
              {assessmentExamEvents.map((examEvent) => (
                <View
                  ...
                  style={[styles.assessmentCard, styles.assessmentCardExam]}
                >
              ...
              {standaloneThesisEvents.map((thesisEvent) => (
                <View
                  ...
                  style={[styles.assessmentCard, styles.assessmentCardThesis]}
                >
                  <View style={styles.assessmentCardHeader}>
                          ...
                          {hasAssignments ? (
                            <Animated.View
                              style={[
                                styles.timeChevronContainer,
                                {
                                  transform: [
                                    {
                                      rotate: isPeriodExpanded ? '180deg' : '0deg',
                                    },
                                  ],
                                },
                              ]}
                            ...
                            <View
                              style={[
                                styles.timeDot,
                                {
                                  backgroundColor: getSubjectColor(item.className),
                                },
                              ]}
                            />
                          )}
                      ...
                      <View style={styles.classContent}>
                        ...
                        <View style={styles.detailsContainer}>
                          <View style={styles.teacherContainer}>
                            ...
                            {item.group &&
                              (item.group === 'Subgroup 1' || item.group === 'Subgroup 2') && (
                                <Text style={styles.groupName}>
                                  {item.group === 'Subgroup 1'
                                    ? t('subgroup').group1
                                    : t('subgroup').group2}
                                </Text>
                              )}
                          ...
                          <View style={styles.roomContainer}>
                            <Text style={styles.roomNumber}>
                              {t('schedule').room} {item.roomNumber}
                            </Text>
                          </View>
                        </View>
                                ...
                                >
                                  <View style={styles.thesisInlineHeader}>
                                    <Ionicons
                                      name="document-text-outline"
                                      size={12}
                                      color={withAlpha(periodColor, 0.95)}
                                    />
                                    <Text style={styles.thesisInlineTitle}>
                                      {t('grades').subjects.thesis}
                                    </Text>
                                    {thesisEvent.subgroup ? (
                                      <Text style={styles.thesisInlineSubgroup}>
                        ...
                        {hasAssignments && isPeriodExpanded && (
                          ...
                          >
                            ...
                            <Text style={styles.assignmentsSectionTitle}>{'Assignments'}</Text>
                                ...
                                >
                                  <View
                                    style={[
                                      ...
                                      {
                                        backgroundColor: getAssignmentTypeColor(
