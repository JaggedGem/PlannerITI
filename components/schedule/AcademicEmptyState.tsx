...
import { GraficPePeriod, GraficPePeriodType, NextScheduledClass } from '@/types/academicCalendar';
...
const periodIcon = (type?: GraficPePeriodType): React.ComponentProps<typeof Ionicons>['name'] => {
  switch (type) {
    case 'exam':
...
export const ExamAgendaCard = memo(function ExamAgendaCard({ events }: ExamAgendaCardProps) {
    ...
    <View style={styles.examAgendaCard}>
      <View style={styles.examAgendaHeader}>
        <View style={styles.examAgendaIcon}>
          <Ionicons name="school-outline" size={18} color={Colors.dark.lightOrange} />
        </View>
        <Text style={styles.examAgendaTitle}>{scheduleText.officialExamSchedule}</Text>
      </View>
      ...
      {sortedEvents.length > 0 ? (
        <View style={styles.examList}>
          {sortedEvents.map((event, index) => (
            <View
              ...
              style={[styles.examRow, index < sortedEvents.length - 1 && styles.examRowBorder]}
            >
              <View style={styles.examDateColumn}>
                <Text style={styles.examDayText}>
                  {formatDate(new Date(`${event.date}T00:00:00`), { weekday: 'short' })}
                </Text>
              ...
              <View style={styles.examDetails}>
                ...
                <Text style={styles.examMeta} numberOfLines={2}>
                  {[
                    formatExamTime(event.time),
                    event.room ? `${scheduleText.room} ${event.room}` : '',
                    event.teacher || '',
                    event.subgroup ? `SG ${event.subgroup}` : '',
                  ]
                    .filter(Boolean)
                    .join(' • ')}
                </Text>
        ...
        <View style={styles.examEmpty}>
          <Text style={styles.examEmptyTitle}>{scheduleText.noOfficialExams}</Text>
          <Text style={styles.examEmptyHint}>{scheduleText.noOfficialExamsHint}</Text>
        </View>
      )}
    </View>
  );
...
const NextClassCard = memo(function NextClassCard({
  ...
  return (
    <TouchableOpacity
      style={[styles.nextClassCard, !nextClass && !isLoading && styles.nextClassCardDisabled]}
    ...
    >
      <View style={styles.nextClassIcon}>
        <Ionicons
          name={nextClass ? 'arrow-forward-circle-outline' : 'calendar-outline'}
          size={24}
          color={nextClass ? Colors.dark.accentBlue : Colors.dark.neutral500}
        />
      </View>
      <View style={styles.nextClassCopy}>
        <Text style={styles.nextClassLabel}>{scheduleText.nextClass}</Text>
        {isLoading ? (
          <Text style={styles.nextClassMuted}>{scheduleText.nextClassLoading}</Text>
        ) : nextClass ? (
          <>
            ...
            <Text style={styles.nextClassSubject} numberOfLines={2}>
              {nextClass.className}
              {nextClass.roomNumber ? ` • ${scheduleText.room} ${nextClass.roomNumber}` : ''}
            </Text>
          </>
        ) : (
          <>
            <Text style={styles.nextClassMuted}>{scheduleText.noUpcomingClasses}</Text>
            <Text style={styles.nextClassHint}>{scheduleText.noUpcomingClassesHint}</Text>
          </>
        )}
      </View>
      {nextClass && <Ionicons name="chevron-forward" size={20} color={Colors.dark.neutral500} />}
    </TouchableOpacity>
  );
});
...
function AcademicEmptyState({
  ...
  const scheduleText = t('schedule');
  const isExamMode = isExamAcademicPeriod(period) || examEvents.length > 0;
  ...
  return (
    <View style={styles.container}>
      <LinearGradient
        colors={[
          isExamMode ? Colors.dark.overlayOrange20 : Colors.dark.surfaceRaisedAlt,
          isExamMode ? Colors.dark.surfaceSecondary : Colors.dark.surfaceSecondary,
        ]}
        start={{ x: 0, y: 0 }}
        end={{ x: 1, y: 1 }}
        style={[styles.heroCard, isExamMode && styles.heroCardExam]}
      >
        <View style={[styles.heroIcon, isExamMode && styles.heroIconExam]}>
          <Ionicons
            name={periodIcon(period?.type)}
            size={28}
            color={isExamMode ? Colors.dark.lightOrange : Colors.dark.primarySoftText}
          />
        </View>
        <View style={styles.heroCopy}>
          ...
          <Text style={[styles.heroSubtitle, isExamMode && styles.heroSubtitleExam]}>
            {isExamMode
              ? variant === 'day'
          ...
          {periodLabel && (
            <View style={styles.periodRow}>
              <View style={[styles.periodPill, isExamMode && styles.periodPillExam]}>
                <Text style={[styles.periodText, isExamMode && styles.periodTextExam]}>
                  {periodLabel}
                </Text>
              </View>
              <Text style={styles.periodRange}>{periodRange}</Text>
              {period?.confidence === 'inferred' && (
                <Text style={styles.inferredText}>{scheduleText.inferredPeriod}</Text>
              )}
            </View>
          )}
        </View>
      ...
      {isExamMode && (variant === 'week' || examEvents.length > 0) && (
