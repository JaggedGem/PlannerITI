                ...
                {visibleCourseKeys.map((courseCode, index) => (
                    ...
                    >
                        <CourseSection
                            ...
                            courseName={
                                courseGroups[courseCode][0].courseName ||
                                'Uncategorized'
                            }
...
const DateGroupedView = memo(
    ({
        ...
        const shouldDelayAnimations =
            isTransitioningFromClasses && isFirstRender;
        ...
        return (
            ...
            >
                ...
                {itemsToRender.map((assignment, index) => (
                    <Animated.View
                        key={assignment.id}
                        entering={FadeInDown.duration(150).delay(
                            (index % 10) * 30,
                        )}
                                ...
                                <TouchableOpacity
                                    ...
                                    style={[
                                        ...
                                        assignment.isCompleted &&
                                            styles.archivedCheckboxCompleted,
                                    ]}
                                >
                                    {assignment.isCompleted && (
                                        <Text style={styles.archivedCheckmark}>
                                            ✓
                                        </Text>
                                    )}
                                </TouchableOpacity>
            ...
            <View style={styles.errorContainer}>
                ...
                {errorCount > 2 && (
                    <Text style={styles.errorHint}>
                        {t('assignments').errors.tip}: Avoid rapid switching
                        between tabs
                    </Text>
                )}
            </View>
...
> {
    ...
    render() {
        if (this.state.hasError) {
            return (
                <AssignmentsErrorFallback errorCount={this.state.errorCount} />
            );
        }
...
const Assignments = () => {
    ...
    const [hasViewedTab, setHasViewedTab] = useState<{
        [key: number]: boolean;
            ...
            const checkNavigationData = async () => {
                try {
                    ...
                    if (navigationDataJson && isMounted) {
                        ...
                        if (
                            navigationData.autoExpand &&
                            navigationData.assignmentId
                        ) {
                            // Store in ref to avoid re-renders when we clear it
                            assignmentToExpandRef.current =
                                navigationData.assignmentId;
    ...
    useEffect(() => {
        const resetHighlightedAssignments = async () => {
            try {
                ...
                const now = Date.now();
                const lastReset = await AsyncStorage.getItem(
                    'last_highlight_reset',
                );
                ...
                console.error(
                    'Error resetting highlighted assignments:',
                    error,
                );
            }
        };
        ...
        (id: string) => {
            ...
            if (isArchived) {
                ...
                setAllAssignments((current) => {
                    ...
                    const priorityAssignments = updated.filter(
                        (a) => a.isPriority,
                    );
                    memoizedDataRef.current = {
                        dateGroups: groupAssignmentsByDate(
                            updated,
                            t,
                            formatDate,
                        ),
    ...
    // todo: Handle archive all past due assignments
        ...
        <SafeAreaView style={styles.container} edges={['top', 'left', 'right']}>
            ...
            <View style={styles.headerContainer}>
                <View style={styles.header}>
                    ...
                    {isSafeMode && (
                        <Text style={styles.safeModeIndicator}>
