...
// Types for grade calculator
type CalculatorTargetType = 'semester' | 'annual';
type CalculatorMode = 'subject' | 'target' | 'results';
type SubjectWithPotentialGrades = GradeSubject & {
...
// Helper function to get grade color - used by both SubjectCard and Calculator
...
const GradeCalculatorModal = ({
  ...
  const { t } = useTranslation();
  const [calculatorMode, setCalculatorMode] = useState<CalculatorMode>('subject');
  const [targetType, setTargetType] = useState<CalculatorTargetType>('semester');
  const [selectedSubjects, setSelectedSubjects] = useState<SubjectWithPotentialGrades[]>([]);
  const [targetAverage, setTargetAverage] = useState('8.00');
  const [numPotentialGrades, setNumPotentialGrades] = useState('1');
  const [allSubjectsWithSameWeight, setAllSubjectsWithSameWeight] = useState(true);
  const [calculationResults, setCalculationResults] = useState<{
    achievable: boolean;
    suggestedGrades: Record<string, number[]>;
  ...
  useEffect(() => {
    if (!visible) {
      setCalculatorMode('subject');
      setTargetType('semester');
      setSelectedSubjects([]);
      setTargetAverage('8.00');
      setNumPotentialGrades('1');
      setAllSubjectsWithSameWeight(true);
      setCalculationResults(null);
    }
  }, [visible]);

  // Handle subject selection
  const toggleSubjectSelection = (subject: GradeSubject) => {
    setSelectedSubjects(prev => {
      ...
      if (isSelected) {
        ...
        return [...prev, {...subject, potentialGrades: [], weight: 1}];
      }
    });
  };

  // Handle subject weight change
  const handleSubjectWeightChange = (subjectName: string, weight: string) => {
    const weightValue = parseFloat(weight);
    if (isNaN(weightValue) || weightValue <= 0) return;

    setSelectedSubjects(prev => 
      prev.map(s => s.name === subjectName ? {...s, weight: weightValue} : s)
    );
  };

  // Calculate current overall average for selected subjects
  const currentAverage = useMemo(() => {
    if (selectedSubjects.length === 0) return '0.00';
    
    if (allSubjectsWithSameWeight) {
      const validAverages = selectedSubjects
      ...
      selectedSubjects.forEach(subject => {
        if (subject.average !== undefined && subject.weight !== undefined) {
  ...
  }, [selectedSubjects, allSubjectsWithSameWeight]);

  // Calculate realistic potential grades based on subject history
  const calculatePotentialGrades = (subject: GradeSubject): number[] => {
    if (!subject.grades || subject.grades.length === 0) {
      // No history, suggest moderate grades (5-9)
      return [5, 6, 7, 8, 9];
    }
    ...
    const mean = numericGrades.reduce((sum, g) => sum + g, 0) / numericGrades.length;
    const variance = numericGrades.reduce((sum, g) => sum + Math.pow(g - mean, 2), 0) / numericGrades.length;
    const stdDev = Math.sqrt(variance);
    
    // Generate realistic grades based on historical performance
    // Center around mean, with variance based on historical performance
    const lowerBound = Math.max(5, Math.round(mean - stdDev));
    const upperBound = Math.min(10, Math.round(mean + stdDev));
    
    const potentialGrades: number[] = [];
    for (let grade = lowerBound; grade <= upperBound; grade++) {
      potentialGrades.push(grade);
    }
    
    return potentialGrades;
  };

  // Move to the next step in the calculator flow
  const moveToNextStep = () => {
    if (calculatorMode === 'subject') {
      // Validate subject selection
      if (selectedSubjects.length === 0) {
        // Show error - no subjects selected
        return;
      }
      
      // Add potential grades to each selected subject
      const subjectsWithPotentialGrades = selectedSubjects.map(subject => ({
        ...subject,
        potentialGrades: calculatePotentialGrades(subject)
      }));
      
      setSelectedSubjects(subjectsWithPotentialGrades);
      setCalculatorMode('target');
    } else if (calculatorMode === 'target') {
      // Validate input
      const targetAverageValue = parseFloat(targetAverage);
      const numGradesValue = parseInt(numPotentialGrades, 10);
      
      if (isNaN(targetAverageValue) || targetAverageValue < 5 || targetAverageValue > 10) {
        // Show error - invalid target average
        return;
      }
      
      if (isNaN(numGradesValue) || numGradesValue < 1 || numGradesValue > 5) {
        // Show error - invalid number of grades
        return;
      }
      
      // Calculate grade combinations needed to reach target
      const result = calculateGradesToReachTarget(
        selectedSubjects,
        targetAverageValue,
        numGradesValue,
        allSubjectsWithSameWeight
      );
      
      setCalculationResults(result);
      setCalculatorMode('results');
    } else {
      // Close modal when done viewing results
      onClose();
    }
  };

  // Calculate grade combinations that would reach the target average
  const calculateGradesToReachTarget = (
    subjects: SubjectWithPotentialGrades[],
    targetAvg: number,
    numGrades: number,
    equalWeights: boolean
  ) => {
    // Current weighted sum and total weight
    let currentWeightedSum = 0;
    let totalWeight = 0;
    
    if (equalWeights) {
      // With equal weights, weight is just the count of subjects
      subjects.forEach(subject => {
        if (subject.average !== undefined) {
          currentWeightedSum += subject.average;
        }
      });
      totalWeight = subjects.length;
    } else {
      // Calculate with specified weights
      subjects.forEach(subject => {
        if (subject.average !== undefined && subject.weight !== undefined) {
          currentWeightedSum += subject.average * subject.weight;
          totalWeight += subject.weight;
        }
      });
    }
    ...
    const currentAvg = totalWeight > 0 ? currentWeightedSum / totalWeight : 0;
    
    // Calculate what the new average components would need to be to reach target
    const targetComponents: Record<string, number[]> = {};
    let isAchievable = true;
    
    // The target weighted sum we need
    const targetWeightedSum = targetAvg * totalWeight;
    
    // The difference we need to make up with new grades
    const requiredDifference = targetWeightedSum - currentWeightedSum;
    
    // Calculate the average grade needed for each subject
    subjects.forEach(subject => {
      // Skip subjects without an existing average
      if (subject.average === undefined) return;
      
      const weight = equalWeights ? 1 : (subject.weight || 1);
      
      // Calculate what new grades we'd need to add to reach the target
      // For simplicity, we'll assume equal distribution of grades
      const requiredGradeSum = requiredDifference * (weight / totalWeight) * numGrades;
      const avgRequiredGrade = subject.average + (requiredGradeSum / numGrades);
      
      // Find grade combinations that get us close to the target
      const suggestedGrades: number[] = [];
      
      // If the average required grade is outside the realistic range, mark as not achievable
      if (avgRequiredGrade < 5 || avgRequiredGrade > 10) {
        isAchievable = false;
      }
      
      // Try to find the best potential grades to recommend
      if (subject.potentialGrades.length > 0) {
        // Function to find how close a grade is to our target
        const closestGrade = (target: number) => {
          return subject.potentialGrades.reduce((prev, curr) => 
            Math.abs(curr - target) < Math.abs(prev - target) ? curr : prev
          );
        };
        
        // Generate numGrades suggested grades
        for (let i = 0; i < numGrades; i++) {
          // Balance the remaining grades needed
          const remainingNeeded = numGrades - i;
          if (remainingNeeded <= 0) break;
          
          const target = (requiredGradeSum - suggestedGrades.reduce((sum, g) => sum + g, 0)) / remainingNeeded;
          const gradeToAdd = closestGrade(target);
          
          suggestedGrades.push(gradeToAdd);
        }
      }
      
      targetComponents[subject.name] = suggestedGrades;
    });
    
    return {
      achievable: isAchievable,
      suggestedGrades: targetComponents,
      targetAverage: targetAvg.toFixed(2),
  ...
  const renderSubjectSelection = () => (
    <View style={styles.calculatorContent}>
      <Text style={styles.calculatorSubtitle}>
        {t('grades').calculator.selectSubjects}
      </Text>
      
      <ScrollView 
        style={styles.calculatorSubjectList}
        showsVerticalScrollIndicator={false}
      >
        {currentSemesterData?.subjects.map((subject, index) => (
          <TouchableOpacity
            key={`calc-subject-${index}`}
            style={[
              styles.calculatorSubjectItem,
              selectedSubjects.some(s => s.name === subject.name) && styles.calculatorSubjectSelected
            ]}
            onPress={() => toggleSubjectSelection(subject)}
          >
            <Text style={styles.calculatorSubjectName}>{subject.name}</Text>
            {subject.displayedAverage && (
              <Text 
                style={[
                  styles.calculatorSubjectAverage,
                  parseFloat(subject.displayedAverage) < 5 && styles.failingGrade
                ]}
              >
                {subject.displayedAverage}
      ...
      <TouchableOpacity
        style={[
          ...
          selectedSubjects.length === 0 && styles.calculatorButtonDisabled
        ]}
        onPress={moveToNextStep}
        disabled={selectedSubjects.length === 0}
      >
        <Text style={styles.calculatorButtonText}>
          {t('grades').calculator.continue}
        </Text>
      </TouchableOpacity>
    </View>
  );

  // Render target input screen
  const renderTargetInput = () => (
    <View style={styles.calculatorContent}>
      <Text style={styles.calculatorSubtitle}>
        {t('grades').calculator.setTarget}
      </Text>
      
      <View style={styles.calculatorForm}>
        {/* Target type selector */}
        <View style={styles.calculatorFormGroup}>
          <Text style={styles.calculatorLabel}>
            {t('grades').calculator.targetType}
          </Text>
          <View style={styles.calculatorToggleGroup}>
            <TouchableOpacity
              style={[
                styles.calculatorToggleButton,
                targetType === 'semester' && styles.calculatorToggleButtonActive
              ]}
              onPress={() => setTargetType('semester')}
            >
              <Text style={[
                styles.calculatorToggleButtonText,
                targetType === 'semester' && styles.calculatorToggleButtonTextActive
              ]}>
                {t('grades').calculator.semester}
              </Text>
            </TouchableOpacity>
            ...
            <TouchableOpacity
              style={[
                styles.calculatorToggleButton,
                targetType === 'annual' && styles.calculatorToggleButtonActive
              ]}
              onPress={() => setTargetType('annual')}
            >
              <Text style={[
                styles.calculatorToggleButtonText,
                targetType === 'annual' && styles.calculatorToggleButtonTextActive
              ]}>
                {t('grades').calculator.annual}
              </Text>
            </TouchableOpacity>
          </View>
        </View>
        ...
        <View style={styles.calculatorFormGroup}>
          ...
          <TextInput
            ...
            value={targetAverage}
            onChangeText={text => {
              // Validate input as decimal between 5 and 10
              const value = text.replace(',', '.');
              if (/^(10|([5-9](\.\d{0,2})?))$|^$/.test(value)) {
                setTargetAverage(value);
              }
            }}
            placeholder="8.00"
            placeholderTextColor="#8A8A8D"
          ...
          />
          <Text style={styles.calculatorHelpText}>
            {t('grades').calculator.currentAverage}: {currentAverage}
          </Text>
        </View>
        
        {/* Number of potential grades input */}
        <View style={styles.calculatorFormGroup}>
          <Text style={styles.calculatorLabel}>
            {t('grades').calculator.numGrades}
          </Text>
          <TextInput
            style={styles.calculatorInput}
            value={numPotentialGrades}
            onChangeText={text => {
              // Validate input as integer between 1 and 5
              if (/^[1-5]$|^$/.test(text)) {
                setNumPotentialGrades(text);
              }
            }}
            placeholder="1"
            placeholderTextColor="#8A8A8D"
          ...
          />
          <Text style={styles.calculatorHelpText}>
            {t('grades').calculator.numGradesHelp}
          </Text>
        </View>
        ...
        <View style={styles.calculatorFormGroup}>
          <Text style={styles.calculatorLabel}>
            {t('grades').calculator.subjectWeights}
          </Text>
          <View style={styles.calculatorToggleGroup}>
            <TouchableOpacity
              style={[
                styles.calculatorToggleButton,
                allSubjectsWithSameWeight && styles.calculatorToggleButtonActive
              ]}
              onPress={() => setAllSubjectsWithSameWeight(true)}
            >
              <Text style={[
                styles.calculatorToggleButtonText,
                allSubjectsWithSameWeight && styles.calculatorToggleButtonTextActive
              ]}>
                {t('grades').calculator.equalWeights}
              </Text>
            </TouchableOpacity>
            ...
            <TouchableOpacity
              ...
              onPress={() => setAllSubjectsWithSameWeight(false)}
            >
              <Text style={[
                styles.calculatorToggleButtonText,
                !allSubjectsWithSameWeight && styles.calculatorToggleButtonTextActive
              ]}>
                {t('grades').calculator.customWeights}
              </Text>
            </TouchableOpacity>
          </View>
        </View>
        
        {/* Custom weights inputs (if enabled) */}
        {!allSubjectsWithSameWeight && (
          <View style={styles.calculatorWeightsList}>
            <Text style={styles.calculatorHelpText}>
              {t('grades').calculator.weightsHelp}
            </Text>
            
            {selectedSubjects.map((subject, index) => (
              <View key={`weight-${index}`} style={styles.calculatorWeightItem}>
                <Text style={styles.calculatorWeightLabel} numberOfLines={1} ellipsizeMode="tail">
                  {subject.name}:
                </Text>
                <TextInput
                  style={styles.calculatorWeightInput}
                  value={subject.weight?.toString() || '1'}
                  onChangeText={text => handleSubjectWeightChange(subject.name, text)}
                  keyboardType="decimal-pad"
                  placeholder="1.0"
                  placeholderTextColor="#8A8A8D"
                />
  ...
  const renderResults = () => {
    ...
    return (
      <View style={styles.calculatorContent}>
        <Text style={styles.calculatorSubtitle}>
          {calculationResults.achievable 
            ? t('grades').calculator.resultsSuccess
            : t('grades').calculator.resultsFailure}
        </Text>
        
        <View style={styles.calculatorResultsSummary}>
          <View style={styles.calculatorResultsRow}>
            <Text style={styles.calculatorResultsLabel}>
              {t('grades').calculator.currentAverage}:
            </Text>
            <Text style={styles.calculatorResultsValue}>
              {calculationResults.currentAverage}
            </Text>
          </View>
          ...
          <View style={styles.calculatorResultsRow}>
            ...
            <Text style={[
              styles.calculatorResultsValue,
              calculationResults.achievable ? styles.calculatorResultsSuccess : styles.calculatorResultsFailure
            ]}>
              {calculationResults.targetAverage}
            </Text>
          </View>
        </View>
        
        <ScrollView style={styles.calculatorResultsList}>
          {Object.entries(calculationResults.suggestedGrades).map(([subjectName, grades], index) => {
            const subject = selectedSubjects.find(s => s.name === subjectName);
            if (!subject) return null;
            
            return (
              <View key={`result-${index}`} style={styles.calculatorResultItem}>
                <Text style={styles.calculatorResultSubject} numberOfLines={2} ellipsizeMode="tail">
                  {subjectName}
                </Text>
                
                <View style={styles.calculatorResultDetails}>
                  <Text style={styles.calculatorResultCurrentAvg}>
                    {t('grades').calculator.current}: {subject.displayedAverage}
                  </Text>
                  
                  <View style={styles.calculatorSuggestedGrades}>
                    <Text style={styles.calculatorResultGradesLabel}>
                      {t('grades').calculator.suggestedGrades}:
                    </Text>
                    
                    <View style={styles.calculatorGradesList}>
                      {grades.length > 0 ? (
                        grades.map((grade, gradeIndex) => (
                          <View 
                            key={`grade-${gradeIndex}`}
                            style={[
                              styles.calculatorGradeItem,
                              { backgroundColor: getGradeColor(grade.toString()) }
                            ]}
                          >
                            <Text style={styles.calculatorGradeText}>
                              {grade}
                            </Text>
                          </View>
                        ))
                      ) : (
                        <Text style={styles.calculatorNoGrades}>
                          {t('grades').calculator.noSuggestions}
                        </Text>
                      )}
                    </View>
                  </View>
    ...
    >
      <View style={styles.modalOverlay}>
        ...
        >
          ...
          {/* Progress indicator */}
          <View style={styles.calculatorProgress}>
            <View style={[
              styles.calculatorProgressStep,
              calculatorMode === 'subject' ? styles.calculatorProgressActive : styles.calculatorProgressCompleted
            ]} />
            <View style={[
              styles.calculatorProgressStep,
              calculatorMode === 'target' 
                ? styles.calculatorProgressActive 
                : calculatorMode === 'results' 
                  ? styles.calculatorProgressCompleted 
                  : styles.calculatorProgressInactive
            ]} />
            <View style={[
              styles.calculatorProgressStep,
              calculatorMode === 'results' ? styles.calculatorProgressActive : styles.calculatorProgressInactive
            ]} />
          </View>
          
          {calculatorMode === 'subject' && renderSubjectSelection()}
          {calculatorMode === 'target' && renderTargetInput()}
          {calculatorMode === 'results' && renderResults()}
        </KeyboardAvoidingView>
      </View>
    </Modal>
  );
...
const styles = StyleSheet.create({
  ...
  calculatorProgressActive: {
    backgroundColor: '#2C3DCD',
  },
  calculatorProgressCompleted: {
    backgroundColor: '#4D96FF',
  },
  calculatorProgressInactive: {
    backgroundColor: '#343434',
  },
  calculatorContent: {
    flex: 1,
  },
  calculatorSubjectList: {
    flex: 1,
    marginBottom: 16,
  },
  ...
  calculatorSubjectSelected: {
    backgroundColor: 'rgba(44, 61, 205, 0.3)',
    borderColor: '#2C3DCD',
    borderWidth: 1,
  },
  calculatorSubjectName: {
    color: 'white',
    fontSize: 16,
    flex: 1,
  },
  calculatorSubjectAverage: {
    color: 'white',
    fontSize: 18,
    fontWeight: 'bold',
    marginLeft: 8,
  ...
  calculatorToggleButtonActive: {
    backgroundColor: '#2C3DCD',
  },
  calculatorToggleButtonText: {
    color: '#8A8A8D',
    fontSize: 14,
  },
  calculatorToggleButtonTextActive: {
    color: 'white',
    fontWeight: '600',
  },
  calculatorWeightsList: {
    marginTop: 8,
    marginBottom: 16,
  },
  calculatorWeightItem: {
    flexDirection: 'row',
    alignItems: 'center',
    marginBottom: 8,
  },
  calculatorWeightLabel: {
    color: 'white',
    fontSize: 14,
    flex: 1,
    marginRight: 8,
  ...
  calculatorResultsValue: {
    color: 'white',
    fontSize: 16,
    fontWeight: 'bold',
  },
  calculatorResultsSuccess: {
    color: '#4CAF50',
  },
  calculatorResultsFailure: {
    color: '#FF6B6B',
  },
  calculatorResultsList: {
    flex: 1,
    marginBottom: 16,
  },
  ...
  calculatorResultCurrentAvg: {
    color: '#8A8A8D',
    fontSize: 14,
    marginBottom: 8,
  },
  calculatorSuggestedGrades: {
    marginTop: 4,
  },
  calculatorResultGradesLabel: {
    color: 'white',
    fontSize: 14,
    marginBottom: 8,
  },
  calculatorGradesList: {
    flexDirection: 'row',
    flexWrap: 'wrap',
    gap: 8,
  },
