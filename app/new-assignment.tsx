...
const DatePicker = ({
    ...
    }, [showCalendar, chevronRotation, calendarOpacity, calendarScale]);
    ...
    const monthAnimStyle = useAnimatedStyle(() => {
        return {
            transform: [
                ...
                {
                    scale: withSpring(monthTransition.value === 0 ? 1 : 0.96, {
                        damping: 15,
                        stiffness: 180,
                    }),
                },
    ...
    const selectQuickOption = React.useCallback(
        (days: number) => {
        ...
        [calendarMonth, selectedDate, onDateChange, monthTransition],
    );
    ...
    const today = new Date();
    const isCurrentDayInView = calendarDays.some(
        (day) => day.isCurrentMonth && isSameDay(day.date, today),
    );
                ...
                <Animated.View style={styles.calendarContainer}>
                    <View style={styles.calendarHeader}>
                        ...
                        <Text style={styles.calendarTitle}>
                            {calendarMonth.toLocaleDateString(currentLanguage, {
                                month: 'long',
                                year: 'numeric',
                            })}
                        </Text>
                    ...
                    <View style={styles.calendarDaysHeader}>
                        {days.map((day, index) => (
                            <Text
                                key={index}
                                style={styles.calendarDayHeaderText}
                            >
                                {day}
                            </Text>
                        ))}
                                    ...
                                    >
                                        <Text
                                            style={[
                                                ...
                                                isToday &&
                                                    !isSelected &&
                                                    styles.calendarDayTextToday, // Only apply today text color if not selected
                                                isSelected &&
                                                    styles.calendarDayTextSelected, // Always apply selected text style last
                                            ]}
                                        >
                                            {day.date.getDate()}
                                        </Text>
...
const TimePicker = ({
    ...
    const hourAnimStyle = useAnimatedStyle(() => {
        return {
            transform: [{ translateY: hourRotation.value }],
        };
    });
    ...
    const minuteAnimStyle = useAnimatedStyle(() => {
        return {
            transform: [{ translateY: minuteRotation.value }],
        };
    });
            ...
            <View style={styles.timeWheelContainer}>
                <View style={styles.timeWheelGroup}>
                    ...
                    <View style={styles.timeWheel}>
                        ...
                        <Animated.Text
                            style={[styles.timeWheelText, hourAnimStyle]}
                        >
                ...
                <View style={styles.timeWheelGroup}>
                    ...
                    <View style={styles.timeWheel}>
                        ...
                        <Animated.Text
                            style={[styles.timeWheelText, minuteAnimStyle]}
                        >
...
export default function NewAssignmentScreen() {
    ...
    const [subjectSelectionMode, setSubjectSelectionMode] = useState<
        'recent' | 'all' | 'custom'
    >('recent');
    ...
    useEffect(() => {
        ...
        const loadSubjects = async () => {
            ...
            try {
                ...
                const subjectsData =
                    (await scheduleService.getSubjects()) as ExtendedSubject[];
                ...
                if (dayIndex !== 0 && dayIndex !== 6) {
                    ...
                    try {
                        ...
                        if (!todaySchedule || !Array.isArray(todaySchedule)) {
                            console.error(
                                'Today schedule is not an array:',
                                todaySchedule,
                            );
                        ...
                        const relevantPeriods = todaySchedule.filter(
                            (period) =>
                                period.isEvenWeek === undefined ||
                                period.isEvenWeek === isEvenWeek,
                        );
                        ...
                        for (const period of relevantPeriods) {
                            ...
                            if (
                                currentTimeInMinutes > endTimeInMinutes &&
                                currentTimeInMinutes <= endTimeInMinutes + 10
                            ) {
                                targetPeriod = period;
                                break;
                            }
                        ...
                        if (!targetPeriod && relevantPeriods.length > 0) {
                            ...
                            const sortedPeriods = [...relevantPeriods].sort(
                                (a, b) => {
                        ...
                        if (targetPeriod && targetPeriod.className) {
                            ...
                            const matchingSubject = subjectsData.find(
                                (subject) =>
                                    subject.name === targetPeriod.className,
                            );

                            if (matchingSubject) {
                                setSelectedSubjectId(matchingSubject.id);
                                setCourseName(matchingSubject.name);
                                updateNextPeriodInfo(matchingSubject.id).catch(
                                    (err) =>
                                        console.error(
                                            'Error updating next period info:',
                                            err,
                                        ),
                                );
                                ...
                                setSelectedSubjectId(subjectsData[0].id);
                                setCourseName(subjectsData[0].name);
                                updateNextPeriodInfo(subjectsData[0].id).catch(
                                    (err) =>
                                        console.error(
                                            'Error updating next period info:',
                                            err,
                        ...
                        console.error(
                            'Error processing today schedule:',
                            error,
                        );
                        if (subjectsData.length > 0) {
                            setSelectedSubjectId(subjectsData[0].id);
                    ...
                    try {
                        ...
                        if (!fridaySchedule || !Array.isArray(fridaySchedule)) {
                            console.error(
                                'Friday schedule is not an array:',
                                fridaySchedule,
                            );
                            if (subjectsData.length > 0) {
                                setSelectedSubjectId(subjectsData[0].id);
                        ...
                        const sortedFridayPeriods = [
                            ...relevantFridayPeriods,
                        ].sort((a, b) => {
                            if (!a.startTime) return 1;
                            if (!b.startTime) return -1;
                        ...
                        console.error(
                            'Error processing Friday schedule:',
                            error,
                        );
                        if (subjectsData.length > 0) {
                            setSelectedSubjectId(subjectsData[0].id);
    ...
    const updateFilteredSubjects = (
        ...
        if (mode === 'recent') {
            ...
            const targetDayIndex =
                dayIndex === 0 || dayIndex === 6 ? 5 : dayIndex;
            ...
            const getTodaySubjects = async () => {
                try {
                    // Get schedule data
                    const scheduleData =
                        await scheduleService.getClassSchedule();
            ...
            try {
                ...
                if (
                    ...
                    for (let i = 1; i <= 7; i++) {
                        ...
                        if (
                            (checkDayIndex === 0 || checkDayIndex === 6) &&
                        ...
                        const adjustedDayIndex =
                            checkDayIndex === 0 ? 7 : checkDayIndex;

                        // If this day is in the period's schedule or the period doesn't specify days
                        if (
                            periodDayIndexes.includes(adjustedDayIndex) ||
                            periodDayIndexes.length === 0
                        ) {
                            dayIncrement = i;
                            foundNextDay = true;
                            break;
                ...
                if (customPeriod.starttime) {
                    const [periodHours, periodMinutes] = customPeriod.starttime
                        .split(':')
                        .map(Number);
                ...
                const isTomorrow =
                    nextDateCopy.getTime() === tomorrow.getTime();
                ...
                if (
                    nextDateCopy >= currentWeekMonday &&
                    nextDateCopy <= currentWeekFriday
                ) {
                    weekText = ''; // Empty for current week
                }
                // Check if the date is between Monday and Friday of next week
                else if (
                    nextDateCopy >= nextWeekMonday &&
                    nextDateCopy <= nextWeekFriday
                ) {
                ...
                // For dates beyond next week
                    ...
                    const weekDifference = Math.ceil(
                        (nextDateCopy.getTime() - currentWeekMonday.getTime()) /
                            (7 * 24 * 60 * 60 * 1000),
                    );
                ...
                console.error(
                    'Error setting next period for custom period:',
                    error,
                );
        ...
        try {
            ...
            for (let i = 0; i <= 14; i++) {
                ...
                if (dayIndex === 0 || dayIndex === 6) {
                    ...
                    const recoveryDay =
                        scheduleService.isRecoveryDay(searchDate);
                ...
                try {
                    ...
                    const subjectPeriod = daySchedule.find(
                        (period) =>
                    ...
                    if (subjectPeriod) {
                        ...
                        const dayString = searchDate.toLocaleDateString(
                            currentLanguage,
                            { weekday: 'long' },
                        );

                        // Format time
                        const formattedTime = formatPeriodTime(
                            subjectPeriod.startTime,
                        );
                        ...
                        // Get the current week's Friday (last day of the week)
                        const currentWeekFriday = new Date(currentWeekMonday);
                        currentWeekFriday.setDate(
                            currentWeekMonday.getDate() + 4,
                        ); // +4 days from Monday is Friday
                        ...
                        tomorrow.setHours(0, 0, 0, 0);
                        const isTomorrow =
                            searchDateCopy.getTime() === tomorrow.getTime();

                        let weekText = '';
                        // Check if the date is between Monday and Friday of the current week
                        if (
                            searchDateCopy >= currentWeekMonday &&
                            searchDateCopy <= currentWeekFriday
                        ) {
                            weekText = ''; // Empty for current week
                        }
                        // Check if the date is between Monday and Friday of next week
                        else if (
                            searchDateCopy >= nextWeekMonday &&
                            searchDateCopy <= nextWeekFriday
                        ) {
                            weekText =
                                t('assignments').days.nextWeek.toLowerCase(); // Lowercase "next week"
                        }
                        // For dates beyond next week
                        else {
                            // Calculate weeks difference for dates beyond next week
                            const weekDifference = Math.ceil(
                                (searchDateCopy.getTime() -
                        ...
                        const thisPrefix =
                            currentLanguage.startsWith('en') ? 'this '
                            : currentLanguage.startsWith('es') ? 'este '
                            : '';
                        const displayDay =
                            isCurrentWeek ?
                                `${thisPrefix}${dayString}`
                            :   dayString;
                        ...
                        const [hours, minutes] = subjectPeriod.startTime
                            .split(':')
                            .map(Number);
                    ...
                    console.error(
                        `Error getting schedule for day ${dayName}:`,
                        error,
                    );
                    continue;
                }
    ...
    const handleSave = async () => {
        if (!title || !courseName || isSaving) {
            if (!title || !courseName) {
                Alert.alert(
                    'Error',
                    'Please enter a title and select a course',
                );
            }
            return;
        }
    ...
    // todo: Render a subject item for the modal
    ...
    const detectAssignmentTypeFromTitle = (text: string) => {
        ...
        const keywordsMap =
            ASSIGNMENT_TYPE_KEYWORDS[currentLanguage as 'en' | 'ro' | 'ru'] ||
            ASSIGNMENT_TYPE_KEYWORDS.en;
        ...
        for (const [type, keywords] of Object.entries(keywordsMap)) {
            for (const keyword of keywords) {
                ...
                const wordPattern = new RegExp(
                    `\\b${keyword}\\b|\\s${keyword}|^${keyword}|${keyword}\\s|${keyword}$`,
                    'i',
                );
                if (
                    wordPattern.test(lowerText) ||
        ...
        for (const lang of Object.keys(ASSIGNMENT_TYPE_KEYWORDS) as (
            | 'en'
            | 'ro'
            | 'ru'
        )[]) {
    ...
    const handleTitleChange = (text: string) => {
        ...
        if (
            !text ||
            (!autoDetectedType && assignmentType !== AssignmentType.HOMEWORK)
        ) {
            return;
        }
        ...
        if (detectedType) {
            ...
            if (
                detectedType === AssignmentType.TEST ||
                detectedType === AssignmentType.EXAM ||
                detectedType === AssignmentType.QUIZ
            ) {
                setIsPriority(true);
            }
        }
    ...
    // todo: Render a type item for selection
    ...
    // todo: handle rendering an icon for each assignment type in the type selector
    ...
    const handleAddButtonPress = () => {
        ...
        // Call actual add function with a slight delay
        setTimeout(() => {
            handleAddSubtask();
        }, 100);
    };
            ...
            <SafeAreaView style={styles.container}>
                ...
                >
                    ...
                    <View style={styles.formContainer}>
                        ...
                        <Animated.View
                            entering={FadeIn.duration(300).delay(75)}
                        >
                            <Text style={styles.label}>
                                {t('assignments').type}
                            </Text>
                        ...
                        >
                            ...
                            <TextInput
                                ...
                                placeholder={
                                    t('assignments').descriptionPlaceholder
                                }
                                placeholderTextColor={Colors.dark.neutral500}
                                value={description}
                            ...
                            >
                                {selectedSubject ?
                                    <View style={styles.selectedSubjectDisplay}>
                                        <Text
                                            style={styles.selectedSubjectText}
                                        >
                                            {selectedSubject.name}
                                        </Text>
                                        {selectedSubject.isCustom && (
                                            <Text
                                                style={styles.customBadgeSmall}
                                            >
                                                {t('assignments').custom}
                                            </Text>
                                        )}
                                    </View>
                            ...
                            >
                                ...
                                {!nextPeriodInfo && selectedSubjectId && (
                                    <View style={styles.warningContainer}>
                                        <Ionicons
                                            name="alert-circle"
                                            size={16}
                                            color={Colors.dark.alertOrange}
                                        />
                                        <Text style={styles.warningText}>
                                            {t('assignments').noUpcomingClasses}{' '}
                                            {selectedSubject?.name ||
                                                t('assignments')
                                                    .thisSubject}{' '}
                                            {t('assignments').inNextTwoWeeks}
                                        </Text>
                                    </View>
                                )}
                                ...
                                {useNextPeriod && (
                                    ...
                                    >
                                        {loadingNextPeriod ?
                                            <View
                                                style={styles.loadingNextPeriod}
                                            >
                                                <ActivityIndicator
                                                    size="small"
                                                    color={
                                ...
                                <View style={styles.subtasksList}>
                                    {subtasks.map((task, index) => (
                                        <Animated.View
                                            ...
                                            exiting={FadeOut.duration(200)}
                                            layout={Layout.springify().damping(
                                                12,
                                            )}
