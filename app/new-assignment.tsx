...
import { CustomPeriod, Subject, scheduleService } from '../services/scheduleService';
import { AssignmentType, addAssignment } from '../utils/assignmentStorage';
  ...
  return (
    <View style={styles.datePickerContainer}>
      <View style={styles.dateSelectionHeader}>
        <TouchableOpacity style={styles.dateDisplay} onPress={() => setShowCalendar(!showCalendar)}>
          <Text style={styles.dateText}>{formatDate(selectedDate)}</Text>
          <Animated.View style={chevronStyle}>
            <Ionicons name="chevron-down" size={20} color={Colors.dark.white} />
          </Animated.View>
        </TouchableOpacity>
      </View>
      ...
      <Animated.View style={calendarContainerStyle}>
        <Animated.View style={styles.calendarContainer}>
          ...
          <View style={styles.calendarDaysHeader}>
            {days.map((day, index) => (
              <Text key={index} style={styles.calendarDayHeaderText}>
                {day}
              </Text>
            ))}
          </View>

          <Animated.View style={[styles.calendarGrid, monthAnimStyle]}>
...
export default function NewAssignmentScreen() {
  ...
  const [subjectSelectionMode, setSubjectSelectionMode] = useState<'recent' | 'all' | 'custom'>(
    'recent',
  );
      ...
      try {
        ...
        if (dayIndex !== 0 && dayIndex !== 6) {
          ...
          try {
            ...
            const todaySchedule = await scheduleService.getScheduleForDay(
              scheduleData,
              dayName as any,
              today,
            );
          ...
          try {
            ...
            const fridaySchedule = await scheduleService.getScheduleForDay(
              scheduleData,
              fridayName as any,
              fridayDate,
            );
  ...
  const updateNextPeriodInfo = async (subjectId: string) => {
    ...
    const subject =
      subjects.find((s) => s.id === subjectId) || filteredSubjects.find((s) => s.id === subjectId);
      ...
      try {
        ...
        // For dates beyond next week
          ...
          const weekDifference = Math.ceil(
            (nextDateCopy.getTime() - currentWeekMonday.getTime()) / (7 * 24 * 60 * 60 * 1000),
          );
        ...
        if (isTomorrow) {
          ...
          const isCurrentWeek =
            nextDateCopy >= currentWeekMonday && nextDateCopy <= currentWeekFriday;
        ...
        try {
          ...
          const subjectPeriod = daySchedule.find(
            (period) =>
              ...
              (i > 0 || (period.startTime && isPeriodInFuture(period.startTime))),
          );
          ...
          if (subjectPeriod) {
            ...
            // Reset all time components to just compare dates
            [currentWeekMonday, currentWeekFriday, nextWeekMonday, nextWeekFriday].forEach(
              (date) => {
                date.setHours(0, 0, 0, 0);
              },
            );
            ...
            // For dates beyond next week
              ...
              // Calculate weeks difference for dates beyond next week
              const weekDifference = Math.ceil(
                (searchDateCopy.getTime() - currentWeekMonday.getTime()) /
                  (7 * 24 * 60 * 60 * 1000),
              );
              weekText = `${t('assignments').days.in} ${weekDifference} ${t('assignments').days.weeks}`;
            }
            ...
            const isCurrentWeek =
              searchDateCopy >= currentWeekMonday && searchDateCopy <= currentWeekFriday;
            // For current week, add "this" before the day, based on current language
            const thisPrefix = currentLanguage.startsWith('en')
              ? 'this '
  ...
  const detectAssignmentTypeFromTitle = (text: string) => {
    ...
    for (const lang of Object.keys(ASSIGNMENT_TYPE_KEYWORDS) as ('en' | 'ro' | 'ru')[]) {
        ...
        >
          ...
          <View style={styles.formContainer}>
            <Animated.View entering={FadeIn.duration(300).delay(50)}>
              ...
              />
              {autoDetectedType && (
                <Text style={styles.autoDetectedNote}>{t('assignments').autoDetectedType}</Text>
              )}
            </Animated.View>
            ...
            <Animated.View entering={FadeIn.duration(300).delay(100)}>
              <Text style={styles.label}>
                {t('assignments').description} ({t('assignments').optional})
              </Text>
              <TextInput
                ...
                {!nextPeriodInfo && selectedSubjectId && (
                  <View style={styles.warningContainer}>
                    ...
                    <Text style={styles.warningText}>
                      ...
                      {selectedSubject?.name || t('assignments').thisSubject}{' '}
                    ...
                    {loadingNextPeriod ? (
                      <View style={styles.loadingNextPeriod}>
                        ...
                        <Text style={styles.nextPeriodText}>
                          {t('assignments').findingNextClass}
                        </Text>
                      </View>
                    ) : nextPeriodInfo ? (
                      <Text style={styles.nextPeriodText}>
            ...
            >
              ...
              <View
                style={[styles.subtaskInputWrapper, { marginTop: subtasks.length > 0 ? 16 : 8 }]}
              >
                      ...
                      customPeriods.length === 0
                        ? () => (
                            <View style={styles.emptyListContainer}>
                              ...
                              >
                                <Text style={styles.settingsLinkButtonText}>
                                  {'Go to Settings'}
                                </Text>
                              </TouchableOpacity>
                            </View>
        ...
        <ModernDropdown
          ...
          items={Object.values(AssignmentType).map((type) => {
            ...
            return {
              ...
              icon: <Ionicons name={getTypeSpecificIcon()} size={24} color={Colors.dark.white} />,
