...
const runCommand = (cmd, cmdArgs, { silent = false } = {}) =>
    new Promise((resolve, reject) => {
        ...
        child.on('exit', (code) => {
            ...
            reject(
                new Error(
                    `bundletool failed (exit ${code})${detail ? `:\n${detail}` : ''}`,
                ),
            );
        });
    });
...
const ask = async (label, fallback = '', isSecret = false) => {
    ...
    const display =
        isSecret ?
            value ? gray('•'.repeat(Math.min(value.length, 10)))
            :   red('(empty)')
        : value ? bold(value)
        : dim('(empty)');
...
const genKeystore = async () => {
    ...
    const outPath = path.resolve(
        await ask('Output path', 'github-release.jks'),
    );
    ...
    const keytoolArgs = [
        ...
        validity,
        '-dname',
        `CN=${cn},OU=Mobile,O=${cn},L=Unknown,ST=Unknown,C=US`,
        '-storepass',
        password,
        '-keypass',
        password,
    ...
    try {
        ...
        /* fingerprint display is optional */
    }
...
const buildTarget = async ({
    ...
    if (universal) {
        ...
        const apksOut = path.join(
            tmpDir,
            `${sanitize(appName)}-universal.apks`,
        );
        ...
        ensureDir(extrDir);
        await runCommand('unzip', ['-q', apksOut, '-d', extrDir], {
            silent: true,
        });

        const selected = pickPreferredApk(findApkFiles(extrDir));
        if (!selected)
            throw new Error('No APK found inside universal .apks archive');
...
const main = async () => {
    ...
    try {
        ...
        const aabPath = path.resolve(
            args.aab || (await ask('Path to .aab bundle')),
        );
        if (!fs.existsSync(aabPath))
            throw new Error(`AAB not found: ${aabPath}`);
        log.info(
            `${gray(aabPath)}  ${dim(formatBytes(fs.statSync(aabPath).size))}`,
        );

        const bundletoolRaw =
            args.bundletool ||
            process.env.BUNDLETOOL_CMD ||
            config.bundletool ||
            'bundletool';
        const bundletoolCmd =
            bundletoolRaw.includes(path.sep) || path.isAbsolute(bundletoolRaw) ?
                path.resolve(bundletoolRaw)
            :   bundletoolRaw;
        ...
        const appName = await ask(
            'App name',
            args.app || config.appName || 'PlannerITI',
        );
        ...
        const outputDir = path.resolve(
            await ask('Output directory', defaultOut),
        );
        ensureDir(outputDir);

        const abiDefault =
            args.abis || config.abis || 'arm64-v8a,armeabi-v7a,x86_64';
        ...
        const includeUniversal =
            ...
            (await ask('Include universal APK?', 'n'))
                .toLowerCase()
                .startsWith('y');
        ...
        const defaultKs =
            args.keystore || process.env.KEYSTORE_PATH || config.keystore || '';
        ...
        const keyAlias = await ask(
            'Key alias',
            args.alias || process.env.KEYSTORE_ALIAS || config.keyAlias || '',
        );
        ...
        const ksPass =
            ...
            (await ask('Keystore password', '', true));
        if (!ksPass) throw new Error('Keystore password is required.');
        const keyPass =
            args['key-pass'] ||
            process.env.KEY_PASS ||
            (await ask('Key password', ksPass, true));

        // Advanced (always default, no prompt spam)
        const locales = normalizeList(
            args.locales || config.locales || 'en,ro,ru',
        );
        const screenDensity = parseInt(
            args.density || config.screenDensity || '480',
            10,
        );
        ...
        // ── Save config ───────────────────────────────────────────────────────────
        if (!args['no-save']) {
            const doSave = (await ask('Save config for next run?', 'y'))
                .toLowerCase()
                .startsWith('y');
            if (doSave) {
                saveConfig({
                ...
                log.success(
                    `Config saved → ${gray('.apkbuilder.json')}  ${yellow('(add to .gitignore!)')}`,
                );
            }
        }
        ...
        const targets = [
            ...
            ...(includeUniversal ?
                [{ label: 'universal', universal: true }]
            :   []),
        ];
        ...
        for (let i = 0; i < targets.length; i++) {
            ...
            spinner.start(
                `[${i + 1}/${targets.length}]  ${t.label}  ${dim('→ .' + ext)}`,
            );
            ...
            try {
                ...
                spinner.succeed(
                    `[${i + 1}/${targets.length}]  ${t.label}  ${gray(formatBytes(fs.statSync(filePath).size))}  ${dim('.' + ext)}`,
                );
                ...
                spinner.fail(
                    `[${i + 1}/${targets.length}]  ${t.label}  ${red(firstLine)}`,
                );
                if (rest)
                    console.error(
                        rest
