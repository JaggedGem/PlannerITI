...
const hRule = (w, l = BOX.tl, r = BOX.tr, fill = BOX.h) => l + fill.repeat(Math.max(0, w - 2)) + r;
...
const ask = async (label, fallback = '', isSecret = false) => {
  ...
  const display = isSecret
    ? value
      ? gray('•'.repeat(Math.min(value.length, 10)))
      : red('(empty)')
    : value
      ? bold(value)
      : dim('(empty)');
...
const main = async () => {
  ...
  try {
    ...
    const bundletoolRaw =
      args.bundletool || process.env.BUNDLETOOL_CMD || config.bundletool || 'bundletool';
    const bundletoolCmd =
    ...
    const defaultVersion =
      args.version || process.env.RELEASE_VERSION || config.version || readPackageVersion();
    ...
    const defaultOut =
      args.out || config.outputDir || path.join(process.cwd(), 'dist', 'apks', version);
    ...
    const ksPass =
      args['ks-pass'] || process.env.KEYSTORE_PASS || (await ask('Keystore password', '', true));
    if (!ksPass) throw new Error('Keystore password is required.');
    const keyPass =
      args['key-pass'] || process.env.KEY_PASS || (await ask('Key password', ksPass, true));
