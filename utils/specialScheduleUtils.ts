...
const normalizeSubgroupValue = (value?: string | null): '1' | '2' | null => {
    ...
    if (
        normalized === '1' ||
        normalized.includes('grupa 1') ||
        normalized.includes('subgroup 1')
    ) {
        return '1';
    }
    if (
        normalized === '2' ||
        normalized.includes('grupa 2') ||
        normalized.includes('subgroup 2')
    ) {
        return '2';
    }
...
const normalizeSelectedSubgroup = (group: SubGroupType): '1' | '2' =>
    group === 'Subgroup 1' ? '1' : '2';

export const getSpecialScheduleDateKey = (date: Date): string =>
    format(date, 'yyyy-MM-dd');

export const eventMatchesSelectedSubgroup = (
    eventSubgroup: string | null | undefined,
...
export const filterThesisEventsForDate = (
    ...
    return events.filter(
        (event) =>
            event.date === dateKey &&
            eventMatchesSelectedSubgroup(event.subgroup, selectedGroup),
    );
};
...
export const thesisMatchesScheduleSlot = (
    ...
    const slotPeriod =
        ...
        :   '';
    const thesisPeriod =
        thesis.period !== null && thesis.period !== undefined ?
            String(thesis.period)
        :   '';
...
export const formatExamTimeLabel = (event: ExamScheduleEvent): string =>
    event.time || '';
