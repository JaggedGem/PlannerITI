...
describe('specialScheduleUtils', () => {
  describe('eventMatchesSelectedSubgroup', () => {
    it('matches events without subgroup to all users', () => {
      ...
      expect(eventMatchesSelectedSubgroup(undefined, 'Subgroup 2')).toBe(true);
    });

    it('matches normalized subgroup labels and rejects mismatch', () => {
      expect(eventMatchesSelectedSubgroup('grupa 1', 'Subgroup 1')).toBe(true);
      expect(eventMatchesSelectedSubgroup('subgroup 2', 'Subgroup 1')).toBe(false);
    });
  });
  ...
  it('filters thesis and exam events by date + subgroup', () => {
    ...
    expect(filterThesisEventsForDate(thesis, date, 'Subgroup 1')).toHaveLength(1);
  ...
  describe('thesisMatchesScheduleSlot', () => {
    it('matches by period when subgroup is compatible', () => {
      ...
      expect(
        thesisMatchesScheduleSlot(event, { period: '3', group: 'subgroup 1' }, 'Subgroup 1'),
      ).toBe(true);
    });
    ...
    it('matches by time when period is unavailable', () => {
      ...
      expect(
        thesisMatchesScheduleSlot(event, { startTime: '12:00', endTime: '13:30' }, 'Subgroup 2'),
      ).toBe(true);
    });
    ...
    it('rejects subgroup mismatch', () => {
      ...
      expect(
        thesisMatchesScheduleSlot(event, { period: 2, group: 'subgroup 1' }, 'Subgroup 2'),
      ).toBe(false);
    });
  });
  ...
  it('formats time labels safely', () => {
    expect(formatThesisTimeLabel(thesisEvent({ startTime: '08:00', endTime: '09:30' }))).toBe(
      '08:00 - 09:30',
    );
