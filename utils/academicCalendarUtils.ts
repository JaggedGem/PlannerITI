import { GraficPePeriod, NextScheduledClass } from '@/types/academicCalendar';
...
export const findAcademicPeriodForDate = (
  ...
  return periods.find((period) => period.startDate <= dateKey && dateKey <= period.endDate) || null;
};

export const academicPeriodSuppressesClasses = (period: GraficPePeriod | null): boolean =>
  Boolean(period && period.type !== 'teaching');

export const isExamAcademicPeriod = (period: GraficPePeriod | null): boolean =>
...
export const isDateInSameWeek = (first: Date, second: Date): boolean =>
  toLocalDateKey(getWeekStart(first)) === toLocalDateKey(getWeekStart(second));

const utcDayNumber = (date: Date): number =>
  Date.UTC(date.getFullYear(), date.getMonth(), date.getDate()) / (24 * 60 * 60 * 1000);

export const getWeekOffsetForDate = (targetDate: Date, anchorDate: Date = new Date()): number => {
  const targetMonday = getWeekStart(targetDate);
  const anchorMonday = getWeekStart(anchorDate);
  return Math.round((utcDayNumber(targetMonday) - utcDayNumber(anchorMonday)) / 7);
};
...
interface FindNextScheduledClassOptions {
  ...
  getScheduleForDate: (date: Date) => SearchableScheduleItem[] | Promise<SearchableScheduleItem[]>;
    ...
    const firstClass = schedule
      .filter(
        (item) =>
          ...
          (item.isEvenWeek === undefined || item.isEvenWeek === evenWeek),
      )
