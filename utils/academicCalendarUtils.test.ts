...
describe('academicCalendarUtils', () => {
  it('matches academic periods inclusively', () => {
    expect(findAcademicPeriodForDate(periods, '2026-01-05')?.type).toBe('inter_semester_break');
    expect(findAcademicPeriodForDate(periods, '2026-01-18')?.type).toBe('inter_semester_break');
    expect(findAcademicPeriodForDate(periods, '2026-01-19')?.type).toBe('teaching');
  });
  ...
  it('identifies exam periods and a shared period scope', () => {
    ...
    expect(findSharedAcademicPeriod([examPeriod, examPeriod])).toBe(examPeriod);
  ...
  it('derives academic years and week navigation offsets locally', () => {
    ...
    expect(isDateInSameWeek(new Date(2026, 5, 8), new Date(2026, 5, 14))).toBe(true);
    expect(getWeekOffsetForDate(new Date(2026, 5, 22), new Date(2026, 5, 8))).toBe(2);
  });
