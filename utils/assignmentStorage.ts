...
export const getAssignments = async (): Promise<Assignment[]> => {
    try {
        ...
        return assignments.map((assignment: any) => ({
            ...assignment,
            assignmentType:
                assignment.assignmentType || AssignmentType.HOMEWORK, // Default to HOMEWORK for older entries
        }));
    } catch (error) {
...
export const addAssignment = async (
    assignment: Omit<Assignment, 'id' | 'isCompleted'>,
): Promise<Assignment> => {
...
export const toggleAssignmentCompletion = async (id: string): Promise<void> => {
    ...
    if (index !== -1) {
        ...
        if (
            ...
            assignments[index].subtasks.length > 0
        ) {
            assignments[index].subtasks = assignments[index].subtasks.map(
                (subtask) => ({
                    ...subtask,
                    isCompleted: true,
                }),
...
export const getSchedulePeriods = async (): Promise<Period[]> => {
    try {
        ...
        const scheduleData =
            await scheduleService.getClassSchedule(selectedGroupId);
...
export const getFormattedPeriods = async (): Promise<
...
export const handleGroupChange = async (
    groupId?: string | SubGroupType,
): Promise<boolean> => {
    try {
        const assignments = await getAssignments();
        ...
        const subjectNames = new Set(
            currentSubjects.map((s) => s.name.toLowerCase()),
        );
...
export const toggleSubtaskCompletion = async (
    ...
    if (assignmentIndex !== -1 && assignments[assignmentIndex].subtasks) {
        const subtaskIndex = assignments[assignmentIndex].subtasks!.findIndex(
            (s) => s.id === subtaskId,
        );
...
export const deleteSubtask = async (
    ...
    if (assignmentIndex !== -1 && assignments[assignmentIndex].subtasks) {
        assignments[assignmentIndex].subtasks = assignments[
            assignmentIndex
...
export const updateAllSubtaskCompletion = async (
    assignmentId: string,
    isCompleted: boolean,
): Promise<void> => {
    const assignments = await getAssignments();
    ...
    if (
        assignmentIndex !== -1 &&
        assignments[assignmentIndex].subtasks &&
        assignments[assignmentIndex].subtasks.length > 0
    ) {
...
export const getSubtasksForAssignment = async (
    assignmentId: string,
): Promise<Subtask[]> => {
    const assignments = await getAssignments();
        ...
        for (const assignment of allAssignments) {
            ...
            if (startTime && endTime && assignment.periodId) {
                const period = periods.find(
                    (p) => p._id === assignment.periodId,
                );
                if (!period) continue;

                // Check if period overlaps with the time range
                if (period.starttime > endTime || period.endtime < startTime)
                    continue;
            }
    ...
    try {
        ...
        return allAssignments.filter((assignment) => {
            ...
            const normalizedCourseCode =
                assignment.courseCode?.trim().toLowerCase() || '';
