...
const lookupEntry = (entry: NextClassCacheEntry | undefined, now: number): NextClassCacheLookup => {
...
export const buildNextClassCacheKey = ({
  ...
  const group =
    normalizeKeyPart(selectedGroupId) || normalizeKeyPart(selectedGroupName) || 'NO_GROUP';
  const periodScope = sharedAcademicPeriod
    ? [
...
export const readNextClassCache = async (
  ...
  try {
    const raw = await AsyncStorage.getItem(`${NEXT_CLASS_CACHE_PREFIX}${cacheKey}`);
    if (!raw) return missingLookup();

    const stored = JSON.parse(raw) as StoredNextClassCacheEntry;
    if (!stored || stored.expiresAt <= now) {
      void AsyncStorage.removeItem(`${NEXT_CLASS_CACHE_PREFIX}${cacheKey}`);
...
export const writeNextClassCache = async (
  ...
  try {
    await AsyncStorage.setItem(`${NEXT_CLASS_CACHE_PREFIX}${cacheKey}`, JSON.stringify(stored));
  } catch {
...
export const clearNextClassCache = async (): Promise<void> => {
  ...
  try {
    ...
    const nextClassKeys = keys.filter((key) => key.startsWith(NEXT_CLASS_CACHE_PREFIX));
