...
const isExpoGoRuntime = (): boolean => {
    const appOwnership = (Constants as { appOwnership?: string | null })
        .appOwnership;
    return (
...
const getNotificationsModule = (): NotificationsModule | null => {
    ...
    try {
        // Lazy load to avoid initializing expo-notifications inside Expo Go.
        // eslint-disable-next-line @typescript-eslint/no-require-imports
        notificationsModuleCache = require('expo-notifications') as NotificationsModule;
        return notificationsModuleCache;
    } catch (error) {
        console.warn(
            'Notifications module unavailable; disabling notifications:',
            error,
        );
        notificationsModuleCache = null;
        return null;
...
export async function registerForPushNotificationsAsync() {
    ...
    const isPermissionGranted = (permissions: unknown): boolean => {
        ...
        const permissionRecord = permissions as {
            granted?: boolean;
            status?: string;
        };
        return (
            permissionRecord.granted === true ||
            permissionRecord.status === 'granted'
        );
    };
    ...
    if (Device.isDevice) {
        ...
        try {
            ...
            console.warn(
                'Push token registration unavailable in current runtime:',
                tokenError,
            );
            return;
        }
...
function getNotificationId(
    assignmentId: string,
    type: 'due' | 'reminder' | 'early-reminder' | 'daily',
) {
...
async function scheduleIndividualNotifications(
    ...
    try {
        ...
        const dueNotificationTime = new Date(
        ...
        const reminderDays = await getReminderDaysForType(
            assignment.assignmentType,
        );
        ...
        const shouldSendDaily = await shouldSendDailyReminder(
            assignment.assignmentType,
        );
        ...
        if (shouldSendDaily) {
            ...
            if (daysUntilDue <= reminderDays && daysUntilDue > 1) {
                // Don't send daily on the last day (we have a 24h notification)
        ...
        if (assignment.isPriority) {
            const priorityReminderTime = new Date(
                dueDate.getTime() - 60 * 60 * 1000,
            ); // 1 hour before
...
function createDigestMessage(
    assignments: AssignmentDigest['assignments'],
): string {
    ...
    const byType = assignments.reduce(
        ...
        {} as Record<
            AssignmentType,
            { title: string; courseInfo: string; dueDate: string }[]
        >,
    );
...
export async function createAndScheduleDailyDigest(
    ...
    try {
        ...
        const incompleteAssignments = assignments.filter(
            (a) => !a.isCompleted && parseISO(a.dueDate) > now, // Exclude assignments past their due date
        );
        ...
        const notificationTime =
            ...
            :   (() => {
                    ...
                    if (todayNotificationTime <= now) {
                        todayNotificationTime.setDate(
                            todayNotificationTime.getDate() + 1,
                        );
                    }
        ...
        for (const [dateKey, dayAssignments] of Object.entries(
            assignmentsByDay,
        )) {
        ...
        const totalAssignments = digestData.reduce(
            (count, day) => count + day.assignments.length,
            0,
        );
        ...
        if (digestData.length === 1) {
            ...
            if (day.assignments.length <= 5) {
                ...
                const countByType = day.assignments.reduce(
                    (acc, curr) => {
                ...
                for (const [type, count] of Object.entries(countByType)) {
                    const typeEmoji = getAssignmentTypeEmoji(
            ...
            for (const day of digestData) {
                ...
                if (day === digestData[0] || day === digestData[1]) {
                    ...
                    const typeGroups = day.assignments.reduce(
                        (acc, curr) => {
...
export async function scheduleNotificationForAssignment(
    assignment: Assignment,
): Promise<void> {
    try {
        // Get notification settings
        ...
        try {
            ...
            console.error(
                'Error reading assignments from storage:',
                storageError,
            );
        }
    } catch (error) {
...
export async function checkAndRescheduleNotifications(
    assignments: Assignment[],
): Promise<void> {
    const Notifications = getNotificationsModule();
    if (!Notifications) {
    ...
    try {
        ...
        scheduledNotifications.forEach((notification) => {
            ...
            const assignmentId =
                typeof assignmentIdValue === 'string' ? assignmentIdValue : (
                    null
                );
        ...
        for (const assignment of incompleteAssignments) {
            ...
            const assignmentTypes =
                scheduledTypes.get(assignment.id) || new Set();
            const daysUntilDue = differenceInDays(dueDate, now);
            const reminderDays = await getReminderDaysForType(
                assignment.assignmentType,
            );
            ...
            if (
                !assignmentTypes.has('early-reminder') &&
                daysUntilDue <= reminderDays &&
                daysUntilDue >= reminderDays - 1
            ) {
