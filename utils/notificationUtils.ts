...
const isExpoGoRuntime = (): boolean => {
  ...
  return Constants.executionEnvironment === 'storeClient' || appOwnership === 'expo';
};
...
const getNotificationsModule = (): NotificationsModule | null => {
  ...
  try {
    ...
    console.warn('Notifications module unavailable; disabling notifications:', error);
    notificationsModuleCache = null;
    return null;
  }
};

export const areNotificationsAvailable = (): boolean => getNotificationsModule() !== null;
...
export async function registerForPushNotificationsAsync() {
  ...
  if (Device.isDevice) {
    ...
    try {
      const projectId =
        process.env.EXPO_PROJECT_ID || Constants.expoConfig?.extra?.eas?.projectId || undefined;
...
function createDigestMessage(assignments: AssignmentDigest['assignments']): string {
  ...
  const byType = assignments.reduce(
    ...
    {} as Record<AssignmentType, { title: string; courseInfo: string; dueDate: string }[]>,
  );
