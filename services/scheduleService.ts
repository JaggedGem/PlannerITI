...
export const scheduleService = {
    ...
    getScheduleRefreshVersion() {
        return this._scheduleRefreshVersion;
    },
        ...
        if (this._initializing) {
            return new Promise((res, rej) => {
                ...
                const timeout = setTimeout(() => {
                    ...
                    rej(
                        new Error(
                            'Timeout waiting for schedule service initialization',
                        ),
                    );
    ...
    makeDatePeriodCacheKey(dateKey: string, groupKey?: string): string {
        const resolvedGroupKey =
            groupKey ?? this.getCurrentPeriodCacheGroupKey();
    ...
    normalizeWeekdayKey(
        weekday?: string,
    ): keyof ApiResponse['data'] | undefined {
    ...
    getRemovedPeriodNumbersForDisplay(
        ...
        const numericRemoved = Array.from(
            new Set(
        ...
        const baseIndices = Array.from(
            new Set((entry.baseSchedule || []).map((period) => period.index)),
        ).sort((a, b) => a - b);
        const activeIndices = Array.from(
            new Set((entry.schedule || []).map((period) => period.index)),
        ).sort((a, b) => a - b);

        const matchesActiveSchedule = (removedIndices: number[]): boolean => {
            if (baseIndices.length === 0) return false;
            const removedSet = new Set(removedIndices);
            const remaining = baseIndices.filter(
                (index) => !removedSet.has(index),
            );
            return (
                remaining.length === activeIndices.length &&
                remaining.every((value, idx) => value === activeIndices[idx])
            );
        };
        ...
        if (zeroBasedMatches && !oneBasedMatches) {
            ...
            const treatAsZeroBased = numericRemoved.some(
                (period) => period === 0,
            );
            displayNumbers =
                treatAsZeroBased ?
    ...
    buildOverrideInfoMessage(entry: DatePeriodTimesEntry | null): string {
        ...
        if (
            entry.dayState === 'empty' ||
            (baseCount > 0 && activeCount === 0)
        ) {
            return baseCount > 0 ?
    ...
    resolveWeekSchedulePeriods(day?: WeekScheduleDay | null): PeriodTime[] {
        ...
        if (
            ...
            return baseSchedule.filter((period) => {
                ...
                return (
                    !removedPeriods.has(periodIndex) &&
                    !removedPeriods.has(periodNumber)
                );
            });
        }
    ...
    normalizeWeekScheduleDay(
        day: WeekScheduleDay | undefined,
        weekStart: Date | null,
        index: number,
    ): DatePeriodTimesEntry | null {
    ...
    buildFallbackApiResponseFromDatePeriodTimes(): ApiResponse {
        ...
        this.getDatePeriodEntriesForCurrentGroup().forEach((entry) => {
            ...
            if (
                ...
                targetDay =
                    this.normalizeWeekdayKey(entry.override.replaceWeekday) ||
                    targetDay;
            }
            ...
            const dayKey =
                entry.isWeekend ? `weekend_${entry.date}` : targetDay;
            ...
            entry.schedule.forEach((period) => {
                ...
                if (!response.data[dayKey][periodNum]) {
                    response.data[dayKey][periodNum] = {
                        both: [],
                        par: [],
                        impar: [],
                    };
        ...
        try {
            ...
            if (cachedData) {
                ...
                const hasValidDays = [
                    ...
                    (day) =>
                        data.data[day] && typeof data.data[day] === 'object',
                );
                ...
                if (cachedRecoveryDays) {
                    try {
                        this.cachedRecoveryDays =
                            JSON.parse(cachedRecoveryDays);
                    } catch {
                        this.cachedRecoveryDays = [];
                    }
                } else {
                    this.cachedRecoveryDays =
                        this.buildRecoveryDaysFromDatePeriodTimes();
                }
                ...
                if (cachedAssignmentCounts) {
                    try {
                        this.cachedAssignmentCounts = JSON.parse(
                            cachedAssignmentCounts,
                        );
                    } catch {
    ...
    normalizeSpecialScheduleDocument(
        ...
        return {
            ...
            filename:
                (
                    typeof (document as { filename?: unknown }).filename ===
                    'string'
                ) ?
            ...
            semester:
                (
                    typeof (document as { semester?: unknown }).semester ===
                    'string'
                ) ?
            ...
            academicYear:
                (
                    typeof (document as { academicYear?: unknown })
                        .academicYear === 'string'
                ) ?
    ...
    getSubjectMatchScore(subject: string, candidate: string): number {
        ...
        const intersectionSize = Array.from(subjectSet).filter((token) =>
        ...
        const lengthPenalty = Math.min(
            0.12,
            Math.abs(normalizedSubject.length - normalizedCandidate.length) /
                80,
        );
        score -= lengthPenalty;
    ...
    getSubgroupPeriodsForSubject(
        ...
        orderedPeriods.forEach(({ period, schedules }) => {
            ...
            const bothItems =
                Array.isArray(schedules.both) ? schedules.both : [];
            const weekItems =
            ...
            items.forEach((item) => {
                ...
                if (
                    this.normalizeSubjectMatchKey(itemSubject) !==
                    normalizedSubject
                )
                    return;

                const subgroup = this.normalizeScheduleSubgroup(
                    item?.groupids?.name,
                );
    ...
    async resolveSubgroupDependentThesisEvents(
        ...
        const hasDependent = thesisEvents.some(
            (event) => event.subgroupDependent,
        );
        if (!hasDependent) return response;

        const baseEvents = thesisEvents.filter(
            (event) => !event.subgroupDependent,
        );
        ...
        try {
            ...
            const normalizedTargetGroup =
                this.normalizeSpecialScheduleGroupName(
            ...
            scheduleData = await this.getClassSchedule(
                resolvedGroupId || '',
                false,
            );
        } catch {
        ...
        for (const groupEvents of grouped.values()) {
            ...
            if (
                missingSubgroups.length === 1 &&
                unassignedPeriods.length === 1
            ) {
        ...
        const events: SpecialScheduleEvent[] = rawEvents
            .map((event: any): SpecialScheduleEvent | null => {
                ...
                const base = {
                    ...
                    teacher:
                        typeof event.teacher === 'string' ? event.teacher : '',
                ...
                return {
                    ...
                    type: 'thesis',
                    period:
                        Number.isFinite(rawPeriod) ? Number(rawPeriod) : null,
                    startTime:
                        (
            ...
            .filter(
                (
                    event: SpecialScheduleEvent | null,
                ): event is SpecialScheduleEvent => Boolean(event),
            );
    ...
    async readSpecialScheduleCache(
        ...
        try {
            ...
            const lastFetchKey = this.getSpecialScheduleLastFetchKey(
                type,
                groupName,
            );
    ...
    async cacheSpecialSchedule(
        type: SpecialScheduleType,
        groupName: string | undefined,
        data: SpecialScheduleResponse,
    ): Promise<void> {
        try {
            const cacheKey = this.getSpecialScheduleCacheKey(type, groupName);
            const lastFetchKey = this.getSpecialScheduleLastFetchKey(
    ...
    async getSpecialSchedule(
        ...
        const sessionKey = this.getSpecialScheduleSessionKey(
            type,
            normalizedGroup,
        );
    ...
    async getExamAndThesisSchedule(
        groupName?: string,
        forceRefresh: boolean = false,
    ): Promise<{
    ...
    async getCachedSpecialSchedule(
        type: SpecialScheduleType,
        groupName?: string,
    ): Promise<SpecialScheduleResponse> {
        await this.ready();
    ...
    async syncPeriodTimesForWeek(
        ...
        if (!response.ok)
            throw new Error('Failed to fetch weekly period times');
            ...
            if (this.settings.selectedGroupId) {
                ...
                if (cachedData) {
                    const transformedData =
                        this.transformScheduleData(cachedData);
    ...
    isRecoveryDay(date: Date): RecoveryDay | null {
        ...
        if (
            !this.cachedRecoveryDays ||
            !Array.isArray(this.cachedRecoveryDays)
        ) {
            return null;
        }

        // Find a matching recovery day that is active and applies to the current group
        const recoveryDay = this.cachedRecoveryDays.find(
            (day) =>
                day.date === dateString &&
                day.isActive &&
                (day.groupId === '' ||
                    day.groupId === this.settings.selectedGroupId),
        );
    ...
    async getScheduleForDay(
        ...
        const datePeriodTimes =
            date ? await this.getDatePeriodTimes(date) : null;
        ...
        const allowedDatePeriods =
            effectiveDateSchedule ?
                new Set(
                    effectiveDateSchedule.map((period) =>
                        String(period.index + 1),
                    ),
                )
            :   null;
        ...
        if (
            ...
            effectiveDateSchedule.forEach((period) => {
                ...
                if (data.assignmentCounts && dateKey) {
                    const countByPeriod = data.assignmentCounts.find(
                        (ac) =>
                ...
                result.push({
                    ...
                    roomNumber: '',
                    isRecoveryDay: Boolean(
                        datePeriodTimes?.isWeekend &&
                        datePeriodTimes?.isOverride,
                    ),
        ...
        Object.entries(daySchedule || {}).forEach(([periodNum, schedules]) => {
            ...
            const processScheduleItem = (
                ...
                const datePeriod = datePeriodTimes?.schedule.find(
                    (p: PeriodTime) => p.index === periodIndex,
                );
                if (datePeriod) {
                ...
                if (data.assignmentCounts && dateKey) {
                    ...
                    if (countByPeriod) {
                        ...
                        const countByCourse = data.assignmentCounts.find(
                            (ac) =>
            ...
            this.settings.customPeriods.forEach((customPeriod) => {
                ...
                if (
                    ...
                    result.push({
                        ...
                        isCustom: true,
                        color:
                            customPeriod.color || Colors.dark.customPeriodColor,
                        assignmentCount,
                    });
                }
            });
    ...
    async updateAssignmentCounts(
        counts: PeriodAssignmentCount[],
    ): Promise<void> {
        try {
        ...
        return !Object.values(data.data).some((daySchedule) => {
            ...
            return Object.values(daySchedule).some((periodSet: any) => {
                ...
                return ['both', 'par', 'impar'].some(
                    (key) =>
                        Array.isArray(periodSet[key]) &&
                        periodSet[key].length > 0,
                );
            });
        });
    },
            ...
            try {
                ...
                if (this.isScheduleDataEmpty(transformed) && targetGroupName) {
                    try {
                        ...
                        this.log(
                            'Retry with refreshed group list failed',
                            retryError,
                        );
                    }
                }
