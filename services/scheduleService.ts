...
interface HistoricAvailabilityMatch {
  academicYear?: string;
  semester?: string;
...
interface HistoricAvailabilityInfo {
  checked?: boolean;
  available?: boolean;
...
export const scheduleService = {
  ...
  specialScheduleInFlight: new Map<string, Promise<SpecialScheduleResponse>>(),
    ...
    try {
      ...
      if (enforceExpiry) {
        ...
        if (
          ...
          Date.now() - lastFetchMs > ACADEMIC_CALENDAR_CACHE_EXPIRY
        ) {
          return null;
        }
      }
  ...
  getHistoricAvailabilityMatches(
    ...
    if (Array.isArray(candidate.historicAvailability)) {
      return candidate.historicAvailability
        .map(normalizeMatch)
  ...
  getHistoricAvailabilityHref(payload: unknown): string | null {
    const matches = this.getHistoricAvailabilityMatches(payload);
    for (const match of matches) {
      const href = normalizeHistoricHref(match.href);
      if (href) return href;
    }
    return null;
  },
  ...
  async fetchJsonWithHistoricFallback(
    ...
    const payload = await this.fetchJsonFromPathOrUrl(target);
    const historicHref = this.getHistoricAvailabilityHref(payload);

    if (
      this.shouldFollowHistoricAvailability(payload) &&
      historicHref &&
      !visited.has(historicHref)
    ) {
      try {
  ...
  buildUnavailableSpecialSchedule(
    ...
    return {
      ...
      reason,
      warnings: [],
      historicAvailability: this.normalizeHistoricAvailability(null),
    };
  },
  ...
  getSubjectMatchScore(subject: string, candidate: string): number {
    ...
    const lengthPenalty = Math.min(
      0.12,
      Math.abs(normalizedSubject.length - normalizedCandidate.length) / 80,
    );
    score -= lengthPenalty;
  ...
  async getSpecialSchedule(
    ...
    const request = (async (): Promise<SpecialScheduleResponse> => {
      try {
        const payload = await this.fetchJsonWithHistoricFallback(requestPath);
      ...
      effectiveDateSchedule.forEach((period) => {
        ...
        if (data.assignmentCounts && dateKey) {
          const countByPeriod = data.assignmentCounts.find(
            (ac) => ac.periodId === periodId && ac.dateKey === dateKey,
          );
          if (countByPeriod) {
        ...
        if (data.assignmentCounts && dateKey) {
          ...
          if (countByPeriod) {
            ...
            const countByCourse = data.assignmentCounts.find(
              (ac) => ac.courseCode === className && ac.dateKey === dateKey,
            );
