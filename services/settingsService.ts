...
class SettingsService {
    ...
    private async init(): Promise<void> {
        ...
        const initialDownloadDone = await AsyncStorage.getItem(
            INITIAL_DOWNLOAD_DONE_KEY,
        );
        ...
        if (this.initialDownloadDone) {
            ...
            this.syncToServerIfNeeded().catch((error) => {
                console.error(
                    'Error checking whether settings sync is needed:',
                    error,
                );
            });
        }
    ...
    async makeAuthRequest(
        ...
        try {
            ...
            console.error(
                `Error making settings request to ${endpoint}:`,
                error,
            );
            throw error;
        }
    ...
    async uploadSettings(): Promise<boolean> {
        ...
        try {
            ...
            const encryptionPassword =
                await authService.getCredentialsForEncryption();
    ...
    private async downloadSettings(): Promise<boolean> {
        ...
        try {
            ...
            const dataLabels = await this.makeAuthRequest(
                '/secure/list-encrypted-data',
                'GET',
            );

            // Data labels are objects with a data_label property, not strings
            if (
                !Array.isArray(dataLabels) ||
                !dataLabels.some(
                    (item) => item.data_label === SETTINGS_DATA_LABEL,
                )
            ) {
            ...
            try {
                ...
                if (settings.userSettings) {
                    this.getScheduleService().updateSettings(
    ...
    async deleteRemoteSettings(): Promise<boolean> {
        ...
        try {
            await this.makeAuthRequest(
                `/secure/delete-encrypted-data/${SETTINGS_DATA_LABEL}`,
                'DELETE',
            );
            return true;
        } catch (error) {
    ...
    private setupAuthStateListener(): void {
        ...
        this.authStateListener = DeviceEventEmitter.addListener(
            'auth_state_changed',
            (event) => {
