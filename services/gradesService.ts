 ...
 */
export const loginWithIdnp = async (
    idnp: string,
    signal?: AbortSignal,
): Promise<any> => {
...
const parseStudentInfo = (html: string): StudentInfo => {
    try {
        ...
        const namePattern =
            /<tr>\s*<th>Numele<\/th>\s*<td>([^<]+)<\/td>\s*<\/tr>/;
        const firstNamePattern =
            /<tr>\s*<th>Prenumele<\/th>\s*<td>([^<]+)<\/td>\s*<\/tr>/;
        const patronymicPattern =
            /<tr>\s*<th>Patronimicul<\/th>\s*<td>([^<]+)<\/td>\s*<\/tr>/;
        const studyYearPattern =
            /<tr>\s*<th>Anul de studii<\/th>\s*<td>([^<]+)<\/td>\s*<\/tr>/;
        const groupPattern =
            /<tr>\s*<th>Grupa<\/th>\s*<td>([^<]+)<\/td>\s*<\/tr>/;
        const specPattern =
            /<tr>\s*<th>Specialitatea<\/th>\s*<td>([^<]+)<\/td>\s*<\/tr>/;
        const curatorPattern =
            /<tr>\s*<th>Diriginte<\/th>\s*<td>([^<]+)<\/td>\s*<\/tr>/;
        const deptHeadPattern =
            /<tr>\s*<th>Șef secție<\/th>\s*<td>([^<]+)<\/td>\s*<\/tr>/;
        const statusPattern =
            /<tr>\s*<th>Statut<\/th>\s*<td>([^<]+)<\/td>\s*<\/tr>/;
        ...
        const yearNumber =
            studyYearStr ? romanToArabic(studyYearStr) : undefined;
...
const parseCurrentGrades = (html: string): SemesterGrades[] => {
    ...
    try {
        const situatiaCurentaPattern =
            /<div[^>]*id="situatia-curenta"[^>]*>([\s\S]*?)<div[^>]*id="note-1"/;
        ...
        const semesterPattern =
            /<div[^>]*class="panel-heading"[^>]*>[\s\S]*?<h4[^>]*>[\s\S]*?<a[^>]*>[\s\S]*?Semestrul\s+([IVX]+|[0-9]+)[\s\S]*?<\/a>[\s\S]*?<\/h4>[\s\S]*?<\/div>[\s\S]*?<div[^>]*id="collaps3e(\d+)"[^>]*>([\s\S]*?)(?=<div[^>]*class="panel-heading"|<div[^>]*id="note-1"|$)/g;
        let panelMatch;
        while (
            (panelMatch = semesterPattern.exec(situatiaCurentaContent)) !== null
        ) {
...
const parseSubjectsFromTable = (
    ...
    let rowPattern =
        /<tr>\s*<td>\s*<p>(.*?)<\/p>\s*<\/td>\s*<td>\s*<p>(.*?)<\/p>\s*<\/td>\s*<\/tr>/gs;
    let rowMatch;
    let found = false;
    ...
    if (!found) {
        ...
        rowPattern =
            /<tr>\s*<td[^>]*>\s*(.*?)\s*<\/td>\s*<td[^>]*>\s*(.*?)\s*<\/td>\s*<\/tr>/gs;
 ...
 * @param semester The semester object to populate
 */
const parseAbsencesFromPanel = (
    panelContent: string,
    semester: SemesterGrades,
): void => {
    const absencesTotalPattern =
        /<tr>\s*<th>Absențe totale<\/th>\s*<th>(\d+)\s*<\/th>\s*<\/tr>/;
    ...
    if (absencesTotalMatch) {
        ...
        const absencesSickPattern =
            /<tr>\s*<td><i>Bolnav<\/i><\/td>\s*<td><i>(\d+)<\/i><\/td>\s*<\/tr>/;
        const absencesMotivatePattern =
            /<tr>\s*<td><i>Motivate<\/i><\/td>\s*<td><i>(\d+)<\/i><\/td>\s*<\/tr>/;
        const absencesNemotivatePattern =
            /<tr>\s*<td><i>Nemotivate<\/i><\/td>\s*<td><i>(\d+)<\/i><\/td>\s*<\/tr>/;
        ...
        semester.absences = {
            ...
            unexcused:
                nemotivateMatch ? parseInt(nemotivateMatch[1].trim(), 10) : 0,
        };
    }
};
...
const parseExams = (html: string): Exam[] => {
    ...
    try {
        ...
        const note1Section = html.match(
            /<div[^>]*id="note-1"[^>]*>(.*?)<div[^>]*id="note-2"/s,
        );
        ...
        const examPanelPattern =
            /<div[^>]*id="collapse(\d+)"[^>]*>(.*?)<\/div>\s*<\/div>/gs;
        let panelMatch;
        ...
        while ((panelMatch = examPanelRegex.exec(note1Content)) !== null) {
            ...
            const examRowPattern =
                /<tr>\s*<td>\s*<p>\(([^)]+)\)\s*([^<]+)<\/p>\s*<\/td>\s*<td>\s*<p>([^<]+)<\/p>\s*<\/td>\s*<\/tr>/g;
            let rowMatch;
            ...
            if (result.length === 0) {
                const altPattern =
                    /<tr>\s*<td>\s*<p>([^<]+)<\/p>\s*<\/td>\s*<td>\s*<p>([^<]+)<\/p>\s*<\/td>\s*<\/tr>/g;
                while ((rowMatch = altPattern.exec(panelContent)) !== null) {
                    ...
                    const name =
                        typeMatch ?
                            fullName.substring(typeMatch[0].length).trim()
                        :   fullName;
...
const parseAnnualGrades = (html: string): AnnualGrade[] => {
    ...
    const annualPanelPattern =
        /<div[^>]*class=["'][^"']*panel-collapse[^"']*["'][^>]*id=["']collaps2e(\d+)["'][^>]*>(.*?)<\/div>\s*<\/div>\s*<\/div>/gs;
    ...
    annualPanels.forEach((panelMatch) => {
        ...
        const gradeRowPattern =
            /<tr[^>]*>\s*<td[^>]*>\s*<p[^>]*>(.*?)<\/p>\s*<\/td>\s*<td[^>]*>\s*<p[^>]*>(.*?)<\/p>\s*<\/td>\s*<td[^>]*>\s*<p[^>]*>(.*?)<\/p>\s*<\/td>\s*<td[^>]*>\s*<p[^>]*>(.*?)<\/p>\s*<\/td>\s*<td[^>]*>\s*<p[^>]*>(.*?)<\/p>\s*<\/td>\s*<td[^>]*>\s*<p[^>]*>(.*?)<\/p>\s*<\/td>\s*<\/tr>/gs;
...
const determineCurrentSemester = (html: string): number | undefined => {
    ...
    const expandedPanelPattern =
        /<div[^>]*class=["'][^"']*panel-collapse\s+in[^"']*["'][^>]*id=["']collaps3e(\d+)["'][^>]*>/;
 ...
 */
const applyExamGradesToAverages = (
    ...
    const currentYearExams = exams.filter(
        (exam) =>
            exam.semester >= firstSemesterThisYear &&
            exam.semester <= lastSemesterThisYear,
    );

    // Some grade tables label semesters as I/II each year (1/2), while exams come as absolute (3/4 for year 2)
    const usesRelativeSemesterNumbers =
        currentYear > 1 &&
    ...
    semesters.forEach((semester) => {
        const effectiveSemesterNumber =
            usesRelativeSemesterNumbers ?
                (currentYear - 1) * 2 + semester.semester
            :   semester.semester;

        // Get exams for this specific semester
        const semesterExams = currentYearExams.filter(
            (exam) => exam.semester === effectiveSemesterNumber,
        );
        ...
        semester.subjects.forEach((subject) => {
            ...
            const matchingExam = findBestExamForSubject(
                subject.name,
                semesterExams,
            );
            ...
            if (matchingExam && subject.baseAverage !== undefined) {
                ...
                if (!isNaN(examGrade)) {
                    ...
                    if (isTeza) {
                        ...
                        newAverage =
                            subject.baseAverage * 0.6 + examGrade * 0.4;
                    } else {
...
export const gradesDataService = {
    ...
    cancelRefresh(idnp?: string) {
        if (idnp) {
            ...
            Object.values(inflightControllers).forEach((controller) =>
                controller.abort(),
            );
        }
    },
    ...
    async store(idnp: string, html: string) {
        ...
        await Promise.all([
            ...
            AsyncStorage.setItem(
                GRADES_CACHE_TIME_KEY_PREFIX + idnp,
                ts.toString(),
            ),
        ]);
        this.notify();
    ...
    async silentRefresh(idnp: string): Promise<{
        updated: boolean;
        html: string | null;
        ...
        if (!inflightRefresh[idnp]) {
            inflightRefresh[idnp] = (async () => {
                ...
                try {
                    const freshHtml = await fetchStudentInfo(
                        idnp,
                        controller.signal,
                    );
                    const currentIdnp = await AsyncStorage.getItem(IDNP_KEY);
                    if (controller.signal.aborted || currentIdnp !== idnp) {
                        if (GRADES_DEBUG_LOGS)
                            console.log(
                                '[grades] refresh aborted or IDNP changed, skipping store',
                            );
                        await Promise.all([
                            AsyncStorage.removeItem(
                                GRADES_CACHE_KEY_PREFIX + idnp,
                            ),
                            AsyncStorage.removeItem(
                                GRADES_CACHE_TIME_KEY_PREFIX + idnp,
                            ),
                        ]);
                        DeviceEventEmitter.emit(GRADES_REFRESH_END_EVENT, {
                            idnp,
                            updated: false,
                            aborted: true,
                        ...
                        if (GRADES_DEBUG_LOGS)
                            console.log('[grades] fetched fresh HTML, storing');
                        await this.store(idnp, freshHtml);
                        if (GRADES_DEBUG_LOGS)
                            console.log('[grades] store complete');
                        DeviceEventEmitter.emit(GRADES_REFRESH_END_EVENT, {
                            idnp,
                            updated: true,
                            duration: Date.now() - startTime,
                        ...
                        if (GRADES_DEBUG_LOGS)
                            console.log(
                                '[grades] fresh HTML empty, keeping cache',
                            );
                        DeviceEventEmitter.emit(GRADES_REFRESH_END_EVENT, {
                            idnp,
                            updated: false,
                    ...
                    if (controller.signal.aborted) {
                        if (GRADES_DEBUG_LOGS)
                            console.log('[grades] silentRefresh aborted');
                        DeviceEventEmitter.emit(GRADES_REFRESH_END_EVENT, {
                            idnp,
                            updated: false,
                            aborted: true,
                        ...
                        if (GRADES_DEBUG_LOGS)
                            console.log('[grades] silentRefresh error', err);
                        DeviceEventEmitter.emit(GRADES_REFRESH_END_EVENT, {
                            idnp,
                            updated: false,
                            error: true,
