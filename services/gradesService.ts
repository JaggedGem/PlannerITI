...
const calculateAverage = (grades: string[]): number | undefined => {
  ...
  const validGrades = grades.map((g) => parseGradeToHundredths(g)).filter((g) => !isNaN(g));
...
const applyExamGradesToAverages = (
  ...
  const currentYearExams = exams.filter(
    (exam) => exam.semester >= firstSemesterThisYear && exam.semester <= lastSemesterThisYear,
  );
  ...
  const usesRelativeSemesterNumbers =
    currentYear > 1 && semesters.some((s) => s.semester <= 2) && exams.some((e) => e.semester > 2);
    ...
    semester.subjects.forEach((subject) => {
      ...
      if (matchingExam && subject.baseAverage !== undefined) {
        ...
        if (!isNaN(examGrade)) {
          ...
          subject.appliedExamType = isTeza
            ? 'teza'
            : isExamen
              ? 'examen'
        ...
        subject.finalDisplayedAverage =
          subject.finalDisplayedAverage ?? subject.baseDisplayedAverage ?? subject.displayedAverage;
      }
    });
  });
};
      ...
      inflightRefresh[idnp] = (async () => {
        ...
        try {
          ...
          if (controller.signal.aborted || currentIdnp !== idnp) {
            if (GRADES_DEBUG_LOGS)
              console.log('[grades] refresh aborted or IDNP changed, skipping store');
            await Promise.all([
              AsyncStorage.removeItem(GRADES_CACHE_KEY_PREFIX + idnp),
              AsyncStorage.removeItem(GRADES_CACHE_TIME_KEY_PREFIX + idnp),
            ]);
